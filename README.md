...
diagnostic from a firm like Alvarez & Marsal, AlixPartners, Hackett Group, FTI
or a Big Four practice is typically a **six-figure engagement running 6–12
...
**Act 2 — the Remediation Loop.** The agentic part. The book is segmented
...
Deterministic math (ingest, diagnose, segment, prioritize) is **never**
delegated to the LLM, so every number is exact and auditable. The LLM is used
only where language and judgment are the point: the narrative and the emails.
All prompts are in [`src/prompts.py`](src/prompts.py) — nothing is hidden.
...
**Roadmap beyond this pilot:** direct ERP connectors; the full cash conversion
cycle (accounts payable and inventory, not AR alone); human-in-the-loop send
with approval queue; and a treasury view that rolls the diagnostic up across
every company in a fund.
